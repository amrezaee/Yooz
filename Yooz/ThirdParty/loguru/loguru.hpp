...
# Version history
    ...
    * Version 1.2.2 - 2016-03-29 - Fix issues with set_fatal_handler throwing an
exception
    * Version 1.2.3 - 2016-05-16 - Log current working directory in loguru::init().
    * Version 1.2.4 - 2016-05-18 - Custom replacement for -v in loguru::init() by
bjoernpollex
    * Version 1.2.5 - 2016-05-18 - Add ability to print ERROR_CONTEXT of parent
thread.
...
().
    ...
    * Version 1.3.0 - 2016-07-20 - Fix issues with callback flush/close not being
called.
    * Version 1.3.1 - 2016-07-20 - Add LOGURU_UNSAFE_SIGNAL_HANDLER to toggle
stacktrace on signals.
...
loguru::create_directories
    ...
    * Version 1.5.0 - 2016-12-22 - LOGURU_USE_FMTLIB by kolis and LOGURU_WITH_FILEABS
by scinart
...
loguru::g_preamble
    ...
    * Version 1.7.2 - 2018-04-22 - Fix a bug where large file names could cause stack
corruption (thanks @ccamporesi)
...
LOGURU_VERBOSE_SCOPE_ENDINGS, add LOGURU_SCOPE_TIME_PRECISION, add named log levels
    * Version 2.0.0 - 2018-09-22 - Split loguru.hpp into loguru.hpp and loguru.cpp
    * Version 2.1.0 - 2019-09-23 - Update fmtlib + add option to loguru::init to NOT
...
// Utility macros
	...
	#define LOGURU_CONCATENATE(s1, s2)      LOGURU_CONCATENATE_IMPL(s1, s2)
	    ...
	    // Helper macro for declaring functions as having similar signature to
	    // printf. This allows the compiler to catch format errors at compile-time.
...
enum NamedVerbosity : Verbosity
{
	...
	// Verbosity levels 1-9 are generally not written to stderr, but are written to
	// file.
	Verbosity_1 = +1,
	Verbosity_2 = +2,
	Verbosity_3 = +3,
...
struct Message
{
	// You would generally print a Message by just concatenating the buffers without
...
/*  Should be called from the main thread.
    ...
    You can you something other than the -v flag by setting the verbosity_flag
   option.
*/
LOGURU_EXPORT
void init(int& argc, char* argv[], const Options& options = {});

// Will call remove_all_callbacks(). After calling this, logging will still go to
// stderr. You generally don't need to call this.
LOGURU_EXPORT
void shutdown();
...
// Returns all arguments given to loguru::init(), but escaped with a single space as
// separator.
LOGURU_EXPORT
...
/*  Will log to a file at the given path.
    ...
    To stop the file logging, just call loguru::remove_callback(path) with the same
   path.
*/
LOGURU_EXPORT
...
/*  Will be called right before abort().
   ...
   exception. Feel free to call LOG:ing function from this, but not FATAL ones! */
LOGURU_EXPORT
...
/*  Will be called on each log messages with a verbosity less or equal to the given
   one. Useful for displaying messages on-screen in a game, for example. The given
   on_close is also expected to flush (if desired).
*/
LOGURU_EXPORT
...
// If g_flush_interval_ms is set to non-zero, this will be called automatically this
// often. If not set, you do not need to call this at all.
LOGURU_EXPORT
void flush();
...
/* Returns the thread name for this thread.
   On most *nix systems this will return the system thread name (settable from both
   within and without Loguru). On other systems it will return whatever you set in
   `set_thread_name()`; If no thread name is set, this will return a hexadecimal
   thread id. `length` should be the number of bytes available in the buffer. 17 is a
   good number for length. `right_align_hex_id` means any hexadecimal thread id will
   be written to the end of buffer.
*/
LOGURU_EXPORT
...
// {
// public:
// 	EcEntryLambda(const char* file, unsigned line, const char* descr, Printer&&
// printer) 		: EcEntryBase(file, line, descr), _printer(std::move(printer)) {}
...
// template<typename Printer>
// EcEntryLambda<Printer> make_ec_entry_lambda(const char* file, unsigned line, const
// char* descr, Printer&& printer)
// {
...
/*
    ...
    The handle is valid as long as the current thread has no changes to its error
   context stack. You can pass the handle to loguru::get_error_context on another
   thread. This can be very useful for when you have a parent thread spawning several
   working threads, and you want the error context of the parent thread to get
   printed (too) when there is an error on the child thread. You can accomplish this
   thusly:
...
// Get a string describing the current stack of error context. Empty string if there
// is none.
LOGURU_EXPORT
...
some.cpp:
    namespace loguru {
        Text ec_to_text(MySmallType small_value)
        {
            ...
            std::string str = small_value.as_string(); // Format 'small_value' here
...
LOGURU_ANONYMOUS_NAMESPACE_END
	...
	#define VLOG_SCOPE_F(verbosity, ...)                                      \
		loguru::LogScopeRAII LOGURU_ANONYMOUS_VARIABLE(error_context_RAII_) = \
		        ((verbosity) > loguru::current_verbosity_cutoff()) ?          \
		                loguru::LogScopeRAII() :                              \
		                loguru::LogScopeRAII(verbosity, __FILE__, __LINE__,   \
		                                     __VA_ARGS__)
...
public:
	...
	void operator&(const AbortLogger&) {}
	    ...
	    /*  Helper functions for CHECK_OP_S macro.
	        GLOG trick: The (int, int) specialization works around the issue that the
	       compiler will not instantiate the template version of the function on
...
/*  GLOG trick: Function is overloaded for integral types to allow static const
   integrals declared in classes and not defined to be used as arguments to CHECK*
   macros. */
template<class T>
